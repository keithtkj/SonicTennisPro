...
class TextToSpeechMaker implements StrokeDataListener {
  ...
  public void strokeDataReceived(TennisStroke stroke) {
    String speechOutput = createSpeechOutputFromStroke(stroke);
    createTTSWavFile(speechOutput);
    // Optionally, play the generated WAV file immediately
  }
  
  // Create a custom speech output based on the TennisStroke
 private String createSpeechOutputFromStroke(TennisStroke stroke) {
        String speech = "Stroke Data: ";
        switch (stroke.getType()) {
            case BallTrajectory:
                speech += "Ball trajectory towards " + stroke.getContext() + " with value " + stroke.getValue();
                break;
            case BallSpeed:
                speech += "Ball speed is " + stroke.getValue();
                break;
            case SwingTiming:
                speech += "Swing timing is " + stroke.getValue();
                break;
            case CourtAmbience:
                speech += "Court ambience level is " + stroke.getValue();
                break;
            default:
                speech += "Unknown stroke type";
        }
        return speech;
    }
